...
public class MineServer
{
	...
	public static void main(String[] args)
	{
		...
		try{
		MineProxy.listen(proxyPort,proxyHTTPPort,proxyHTTPSPort,VERSION);
		...
		String load;
		try{load=args[0];}
		catch(ArrayIndexOutOfBoundsException e){load="minecraft_server.jar";}
		...
		if(classpath!=null)
		{
			...
			ClassPathUpdater.add(load);
			for(String entry:classpathEntries)
			{
				ClassPathUpdater.add(entry);
			}
		}
		*/
		...
		}catch(Exception e){System.out.println("Error loading class "+name+" from jar "+load+":");e.printStackTrace();System.exit(1);}
		String[] nargs;
		try{
		nargs=new String[args.length-1];
		System.arraycopy(args,1,nargs,0,nargs.length);
		}catch(Exception e){nargs=new String[0];}
